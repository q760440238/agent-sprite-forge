...
class SplitGridTests(unittest.TestCase):
    def test_non_divisible_grid_pads_right_and_bottom_without_dropping_pixels(self) -> None:
        image = Image.new("RGBA", (10, 10), (0, 0, 0, 0))
        image.paste(SUBJECT, (9, 8, 10, 10))

        padded, padding = MODULE.pad_grid_canvas(image, rows=2, cols=3)
        ...
        self.assertEqual(padded.size, (12, 10))
        self.assertEqual(padding, {"right": 2, "bottom": 0})
        self.assertIsNotNone(frames[-1].getbbox())
        self.assertEqual(info[-1]["source_box"], [8, 5, 12, 10])

    def test_grid_rows_and_columns_must_be_positive(self) -> None:
        with self.assertRaisesRegex(ValueError, "greater than zero"):
            MODULE.pad_grid_canvas(Image.new("RGBA", (8, 8)), rows=0, cols=2)

    def test_process_records_grid_normalization_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            args = MODULE.build_parser().parse_args(
                [
                    "process", "--input", str(source), "--target", "creature",
                    "--mode", "idle", "--output-dir", str(output_dir),
                    "--rows", "2", "--cols", "3", "--trim-border", "0",
                    "--edge-clean-depth", "0",
                ]
            )

            MODULE.cmd_process(args)
            metadata = json.loads((output_dir / "pipeline-meta.json").read_text())

        self.assertEqual(metadata["grid_normalization"]["original_size"], [10, 10])
        self.assertEqual(metadata["grid_normalization"]["normalized_size"], [12, 10])
        self.assertEqual(metadata["grid_normalization"]["padding"], {"right": 2, "bottom": 0})

    def test_animated_webp_preserves_each_frame_alpha(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            frames = []
            for offset in (0, 1):
                frame = Image.new("RGBA", (8, 8), (0, 0, 0, 0))
                frame.putpixel((2 + offset, 2), (60, 120, 220, 128))
                frame.putpixel((3 + offset, 3), (60, 120, 220, 255))
                frames.append(frame)
            output = root / "animation.webp"
            MODULE.save_transparent_webp(frames, output, duration=100)
            ...
            with Image.open(output) as animation:
                ...
                for index in range(animation.n_frames):
                    ...
                    self.assertIn(128, alpha.get_flattened_data())
...
class ParserTests(unittest.TestCase):
    def test_default_prompt_rules_are_medium_neutral(self) -> None:
        prompt, _seed = MODULE.build_prompt("creature", "idle", "moonlit wolf guardian")

        self.assertNotIn("pixel art", prompt.lower())
        self.assertNotIn("digimon", prompt.lower())
        self.assertNotIn("not cute", prompt.lower())

    def test_custom_grid_prompt_declares_exact_frame_contract(self) -> None:
        prompt, _seed = MODULE.build_prompt(
            "creature",
            "idle",
            "moonlit wolf guardian",
            rows=4,
            cols=6,
        )

        self.assertIn("EXACTLY 24 equal-size cells", prompt)
        self.assertIn("4 rows and 6 columns", prompt)
        self.assertIn("continuous idle sequence", prompt)
        self.assertNotIn("2x2 pixel art idle", prompt)

    def test_custom_grid_requires_both_dimensions(self) -> None:
        with self.assertRaisesRegex(ValueError, "both rows and cols"):
            MODULE.build_prompt("creature", "idle", "wolf guardian", rows=2)
