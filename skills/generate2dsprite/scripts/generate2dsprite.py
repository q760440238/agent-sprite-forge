...
ART_STYLE = (
    "Original 2D game-ready subject with a clear, readable silhouette and "
    "consistent proportions across frames. Fully transparent background (PNG alpha channel), "
    "no background fill, no checkerboard pattern, no gradients. "
    "NO text, NO labels, NO words, NO letters anywhere."
)

CHAR_STYLE = (
    "Top-down 2D RPG character, 3/4 view from slightly above: show the top of "
    "the head, shoulders, and full body. Keep a readable game-ready silhouette. "
    "Character fills ~60% of its cell with margin for the engine to render cleanly. "
    "Fully transparent background (PNG alpha channel), no background fill, "
    "no checkerboard pattern, no shadow under character. "
...
GRID_RULES = (
    ...
    "5. Space between quadrants is fully transparent, nothing else."
)
...
GRID_RULES_4X4 = (
    ...
    "5. Character is CENTERED horizontally and vertically within its cell. Fills ~60% of the cell, leaving equal empty margin on all four sides. "
    "6. Space between cells is fully transparent (PNG alpha), nothing else."
)
...
def build_custom_grid_prompt(
    ...
    """Build one unambiguous animation prompt for a user-selected grid."""
    if rows < 1 or cols < 1 or rows * cols > 64:
        raise ValueError("Custom grids must be between 1 and 64 frames.")

    if target == "player":
        subject = f"top-down RPG hero. CHARACTER: {prompt}"
    elif target == "npc":
        ...
        subject = f"top-down RPG NPC. ROLE: {NPC_ROLES[role]}. VISUAL DETAILS: {prompt}"
    elif target == "creature":
        subject = f"game creature. SUBJECT: {prompt}"
    else:
        subject = f"2D game asset. SUBJECT: {prompt}"

    style_rules = CHAR_STYLE if target in {"player", "npc"} else ART_STYLE
    frame_count = rows * cols
    if frame_count == 1:
        return (
            f"Single transparent-background {subject}, centered with generous margin. "
            f"Readable game-ready silhouette. NO text, NO labels, NO UI. {style_rules}"
        )
    return (
        f"A {rows}x{cols} 2D game sprite animation sheet of the same {subject}. "
        f"Animation intent: one continuous {mode} sequence. "
        f"SHEET CONTRACT: EXACTLY {frame_count} equal-size cells arranged in {rows} rows and {cols} columns. "
        "Read cells left-to-right, top-to-bottom as consecutive animation frames. "
        "Every cell shows the same subject identity, camera distance, standing-equivalent scale, and palette. "
        "Keep the subject inside its cell with transparent padding; do not add borders, dividers, text, labels, numbers, arrows, UI, or a background. "
        f"Use fully transparent PNG alpha wherever the asset is absent. {style_rules}"
    )
...
def build_prompt(
    ...
    if rows is not None and cols is not None:
        return build_custom_grid_prompt(target, mode, prompt, role, rows, cols), seed

    if target == "creature":
        if mode == "single":
            result = f"Single 2D game creature sprite, centered, facing right. {prompt}. {ART_STYLE}"
        elif mode == "evolution":
            descs = build_evolution_descs(prompt, rng)
            result = (
                f"A 2x2 2D game image showing 4 evolution stages of {prompt}. "
            ...
            result = (
                f"A 2x2 2D game idle animation sheet of the same {prompt}. "
            ...
            result = (
                f"A 2x2 2D game combat sheet of the same {prompt}. "
            ...
            result = (
                f"A 2x2 2D game sprite sheet of the same {prompt} in 4 poses. "
            ...
            result = (
                f"A 2x2 2D game sprite sheet of a walk cycle of the same {prompt}. "
        ...
        if mode == "player":
            result = (
                ...
                "Front-facing (toward camera), idle standing pose, centered in canvas with lots of transparent margin around. "
                f"{CHAR_STYLE}"
            )
        elif mode == "player_walk":
            result = (
                "A 2x2 2D game sprite sheet of a top-down RPG hero walk cycle, ALL FRAMES FACING DOWN (toward camera). "
                f"CHARACTER: {prompt}. "
            ...
            result = (
                "A 4x4 2D game sprite sheet, full 4-direction walk cycle for a top-down RPG hero. "
            ...
            result = (
                "A 2x2 2D game sprite sheet of a top-down RPG hero in 4 action states, all facing DOWN (toward camera). "
                f"CHARACTER: {prompt}. "
        ...
        if mode == "npc":
            ...
            result = (
                "A 2x2 2D game sprite sheet, top-down RPG NPC walk cycle, ALL FRAMES FACING DOWN (toward camera). "
                f"ROLE: {role_desc}. "
                f"VISUAL DETAILS: {prompt}. "
                "Top-left: neutral standing, both feet together. "
        ...
        if mode == "single":
            result = (
                f"Single 2D game asset sprite. SUBJECT: {prompt}. "
                "Centered in the canvas with clear transparent margin around it. "
                "Readable silhouette, game-ready shape consistency, transparent PNG alpha background. "
                f"{ART_STYLE}"
            )
        else:
            rows, cols = GRID_SHAPES.get(mode, (2, 2))
            result = (
                f"A {rows}x{cols} 2D game animation sheet of the same {prompt}. "
                "The same asset identity appears in every cell, with the same on-screen scale, "
...
def normalize_alpha(img: Image.Image, threshold: int = 100, edge_threshold: int = 150) -> Image.Image:
    """Ensure the sheet has a usable alpha channel.

    The image model now returns real RGBA, so in the common case this is a
    no-op. If the sheet came back fully opaque (older models, user uploads),
    fall back to flood-filling the flat background colour sampled at the
    corners.
    """
...
def remove_bg_magenta(img: Image.Image, threshold: int = 100, edge_threshold: int = 150) -> Image.Image:
    """Backward-compatible name for integrations written before RGBA output."""
    return normalize_alpha(img, threshold, edge_threshold)
...
def pad_grid_canvas(
    ...
    """Pad only the right and bottom edges so every grid cell is whole-sized."""
    if rows <= 0 or cols <= 0:
        raise ValueError("Grid rows and cols must be greater than zero.")
    width, height = img.size
    padded_width = math.ceil(width / cols) * cols
    padded_height = math.ceil(height / rows) * rows
    padding = {
...
def center_single_sprite(img: Image.Image, size: int, threshold: int, edge_threshold: int) -> Image.Image:
    cleaned = normalize_alpha(img.convert("RGBA"), threshold, edge_threshold)
...
def split_grid(
    ...
    padded, _padding = pad_grid_canvas(img.convert("RGBA"), rows, cols)
    cleaned = normalize_alpha(padded, threshold, edge_threshold)
    width, height = cleaned.size
...
def save_transparent_webp(frames: list[Image.Image], out_path: Path, duration: int) -> None:
    """Encode an animated WebP without reducing the source alpha channel.

    GIF supports only one transparent palette entry, so anti-aliased sprite
    edges become jagged. Animated WebP preserves each frame's RGBA pixels.
    """
    if not frames:
...
def cmd_process(args: argparse.Namespace) -> None:
    ...
    if has_custom_grid or args.mode in GRID_SHAPES:
        ...
        raw, grid_padding = pad_grid_canvas(raw, rows, cols)
        raw.save(out_dir / "raw-sheet.png")
        cleaned = normalize_alpha(raw.copy(), args.threshold, args.edge_threshold)
        ...
        if args.mode == "player_sheet" and not has_custom_grid and (rows, cols) == (4, 4):
            ...
            for row_index, direction in enumerate(directions):
                ...
                save_transparent_webp(row_frames, out_dir / f"{direction}.webp", args.duration)
            metadata["directions"] = directions
        else:
            save_transparent_webp(frames, out_dir / "animation.webp", args.duration)

        metadata["rows"] = rows
        metadata["cols"] = cols
        metadata["grid_normalization"] = {
            "original_size": list(original_size),
            "normalized_size": list(raw.size),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    build_prompt_parser.add_argument("--rows", type=int)
    build_prompt_parser.add_argument("--cols", type=int)
