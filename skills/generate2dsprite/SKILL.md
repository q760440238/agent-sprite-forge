...
- Use the script only as a deterministic processor: alpha normalization, frame splitting, component filtering, scaling, alignment, QC metadata, transparent sheet export, and GIF export.
...
- Keep the fully transparent background rule unless the user explicitly wants a different processing workflow. The image model returns real RGBA, so no chroma key step is needed; the processor only falls back to sampling a flat background colour when the input sheet is fully opaque.
...
- Still require exact sheet shape, transparent background, frame containment, and same scale across frames.
...
- Keep the character centered in every cell. The body centerline should stay near the cell center, feet/bottom anchor should stay on the same y-position when visible, and the subject should occupy only the central safe area with generous transparent padding.
...
- frame PNGs
- `animation.webp` (animated WebP with full alpha)
- `prompt-used.txt`
- `pipeline-meta.json`
