...
# 只生成新增的写实和恐怖画风
NEW_STYLES = [
    s for s in ART_STYLES if s.get('group') in ['写实', '恐怖']
]
...
async def main():
    ...
    print(f"开始为 {len(NEW_STYLES)} 个新画风生成预览图...")
    print(f"  - 写实风格: 20 个")
    print(f"  - 恐怖风格: 20 个")
    print("=" * 60)
    
    batch_size = 5
    batches = [NEW_STYLES[i:i+batch_size] for i in range(0, len(NEW_STYLES), batch_size)]
    ...
    print(f"\n{'='*60}")
    print(f"总结: {total_success}/{len(NEW_STYLES)} 成功生成")
    if all_failed:
